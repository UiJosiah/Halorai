...
def ai_flyer():
  ...
  ref_order_lines: list[str] = [
    ...
    "  Study every text element in the template: size/weight hierarchy, color, glow, outline/stroke, drop shadow, gradient fills, 3D/bevel, metallic shine, emboss, and any decorative treatment on the sample wording.",
    "  Render USER CONTENT using the SAME text effects as the matching template blocks — never as plain, flat, or boring default typography.",
    "  Do NOT use the template as the final background. Remove template placeholder faces and template sample wording from the final output (replace with USER CONTENT in the same styles).",
  ...
  prompt_lines = [
    ...
    *ref_order_lines,
    ...
    "- For each USER CONTENT field, find the closest matching text block in the template and copy that block’s FULL visual treatment: type feel, weight, color, glow, outer glow, drop shadow, stroke/outline, gradient fill, texture, 3D/extrusion, bevel, metallic shine, emboss, or any decorative effect visible on the template sample text.",
    "- FORBIDDEN: plain white/black flat text, generic system fonts, or unstyled body copy — unless that exact slot in the template is intentionally minimal.",
    "- Swap only the words to USER CONTENT (spell exactly); keep the template’s text effects, sizing relationship, and placement for that slot.",
    "- RENDERING HIERARCHY (STRICT): The event THEME must use the template’s dominant headline treatment (largest / most effect-heavy primary text style in reference #2).",
    "- The EVENT NAME must use a clearly secondary template style — smaller, lighter, or less effect-heavy than the theme. Never outrank the theme.",
    "- Date, time, and venue use the template’s supporting-detail styles; stay legible and integrated with the same effects family as the template.",
    "- Spell every USER CONTENT string exactly as given (no paraphrasing, no spelling changes).",
    "- Ensure effects remain readable on top of background #1 (adjust glow/shadow strength if needed, but keep the same effect type as the template).",
    "",
    "2. Minister photos",
    minister_slot_summary,
